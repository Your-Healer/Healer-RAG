...
class PGVectorStore:
    ...
    def __init__(self, connection_string=None, embedding_function=None, collection_name="healer_medical_docs"):
        """
    ...
    def _initialize_store(self):
        """Initialize the PGVector store."""
        try:
            # Updated to use connection_url instead of connection_string
            self.store = PGVector(
            ...
            logger.info(f"PGVector store initialized with collection: {self.collection_name}")
        except Exception as e:
    ...
    def add_texts(self, texts: List[str], metadatas: List[Dict[str, Any]] = None) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Added {len(texts)} texts to PGVector store")
            return ids
        except Exception as e:
            logger.error(f"Error adding texts to PGVector: {str(e)}")
            raise
    ...
    def similarity_search_with_filter(self, query: str, filter: Dict[str, Any], k: int = 4) -> List[Document]:
        ...
        try:
            ...
            logger.error(f"Error in similarity search with filter: {str(e)}")
            raise
...
def get_pg_connection_string(
    ...
    host = host or os.environ.get("PGVECTOR_HOST", "localhost")
    port = port or os.environ.get("PGVECTOR_PORT", "5432")
    database = database or os.environ.get("PGVECTOR_DATABASE", "healer_vector_db")
    user = user or os.environ.get("PGVECTOR_USER", "postgres")
    password = password or os.environ.get("PGVECTOR_PASSWORD", "")
...
def create_pg_vector_store(
    ...
    collection_name="healer_medical_docs"
):
    """
